...
claude_context:
  ...
  forbidden_paths:
    - ".env"
    - "alembic/versions/"     # 迁移文件由 Claude Code 自动生成，但不得删除历史
  conventions: |
    - 所有 ID 使用 UUID 字符串
    - 金额使用 Numeric(15,2)
    - 时间戳使用 now_cst()
...
│  │  PostgreSQL · Redis · Celery · Docker/Podman · Git               │  │
│  └──────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────┘
```
...
Claude Code 创建 worktree（占用一个并发槽位）
         ↓
Preview Orchestrator 监听到新 worktree
         ↓
快照预览数据库 → 构建镜像 → 启动容器 → 注入快照数据库
         ↓
健康检查通过 → 预览 URL 就绪
         ↓
推送通知给管理员："预览已就绪，点击查看"
...
Claude Code 持续修改代码
         ↓
文件变更监听 → 热重载（默认，无需重启容器）
         ↓
管理员审核时实时看到最新效果
（管理员可随时点击「重启容器」强制全量重载）
         ↓
审核通过：合并到 dev → dev 预览更新 → worktree 预览销毁
          → 快照数据库同步删除 → 并发槽位释放
审核拒绝：worktree 销毁 → 容器停止 → 快照数据库删除 → 槽位释放
```

### 5.3 预览数据要求与数据库策略

每个 worktree 预览使用独立的数据库，避免预览操作污染真实数据。
...
Review Portal 通过 WebSocket 实时推送所有状态变化：
...
projects
  ...
  repo_url, repo_branch_dev, repo_branch_main
  config_snapshot         -- autoforge.yaml 快照（jsonb）
...
issue_entries
  id, project_id
  source_type             -- widget | admin | github | monitor | scan
  source_id, source_url
  title, description
  category                -- Bug | Feature | Improvement | Security | Debt
  severity                -- critical | high | medium | low
  priority                -- 1-10
  status                  -- 待分析→待审核→已批准/拒绝→实现中→已完成
  fingerprint             -- 去重哈希
  -- 预留：Widget 来源隐私扩展字段
  submitter_ip_hash       -- nullable，IP 哈希（不存原始 IP）
  attachment_urls         -- nullable，截图等附件（预留隐私处理钩子）
...
issue_analyses
  ...
  duplicate_of            -- FK → issue_entries.id
  affected_modules        -- jsonb: 预估影响模块
  analysis_summary        -- ≤200 字摘要
  raw_llm_output          -- jsonb 完整输出
  created_at
...
preview_environments
  ...
  container_id            -- Docker/Podman 容器 ID
  preview_url             -- 访问地址
...
test_sessions
  ...
  summary                 -- 一句话摘要
  results_json            -- 详细结果
  issues_created          -- jsonb: 自动创建的 issue IDs
  started_at, completed_at
...
| 组件 | 选型 | 理由 |
|------|------|------|
| 工厂后端 | FastAPI + Python | 成熟异步生态，与 Claude API 集成方便 |
| 任务调度 | Celery + Redis | 成熟的分布式任务队列 |
| 数据库 | PostgreSQL | 结构化 + JSONB 灵活字段 |
| 容器管理 | Docker / Podman API | 跨平台，预览容器生命周期管理 |
| 预览路由 | Nginx reload（初期）| 按 cr-id 路由到对应容器；高频场景下 reload 可能导致短暂中断，届时评估 Traefik |
| Claude Code 集成 | Claude API + worktree CLI | 原生工具调用 |
| Review Portal | React + WebSocket | 实时状态推送，响应式布局 |
| 通知 | SMTP + Slack Webhook | 覆盖主流通知渠道 |
...
| 阶段 | 内容 | 预计工作量 |
|------|------|----------|
| M1 | 工厂基础骨架：项目注册 + 数据模型 + Review Portal 框架 | 5–7 天 |
...
| M5 | **预览系统核心**：容器管理 + Nginx reload 路由 + **seed 脚本数据库**（仅此路径）| 7–10 天 |
...
| 预览路由实现 | 先用 **Nginx reload**，出现性能瓶颈时再评估 Traefik |
