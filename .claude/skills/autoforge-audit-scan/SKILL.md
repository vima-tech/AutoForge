...
→ execution (Claude Code in a git worktree) → **review_2 (human)** → **pre-merge test
gate** → merge to `dev` → post-merge security audit. AI agents run via local `claude`
...
- **Sinks:** `GitProxy` (git), `sh -lc` test/dev commands, worktree filesystem,
  `.autoforge/` workspace writes, SQLite, the `claude` CLI prompt, external LLM HTTP
...
5. **The code agent cannot touch remotes.** Worktree `claude` runs with
