...
import { AGENTS, LLM_CONFIGS, type LLMConfig, type Agent } from '../data/mock';
...
function LLMSettings() {
  const [configs, setConfigs] = useState<LLMConfig[]>(LLM_CONFIGS);
  const [exp, setExp] = useState<string | null>('l1');
  return (
          ...
          {exp === c.id && (
            <div className="cfg-fields rise">
              <div className="field">
                ...
                <select defaultValue={c.provider}>
              ...
              <div className="field full">
                <label><Icon name="key" size={11} style={{ verticalAlign: -1, marginRight: 4 }} />API Key</label>
                <input className="mono" defaultValue={c.key} type="text" />
              </div>
      ...
      <div className="panel" style={{ marginBottom: 22 }}>
        ...
        <div style={{ padding: '4px 18px 14px' }}>
          <div className="assign-row">
            ...
            <select style={{ width: 180, background: 'var(--bg-3)', border: '1px solid var(--border-strong)', borderRadius: 9, padding: '8px 10px', color: 'var(--text)', fontSize: 13 }} value={analystId} onChange={e => setAnalystId(e.target.value)}>
          ...
          <div className="assign-row">
            ...
            <select style={{ width: 180, background: 'var(--bg-3)', border: '1px solid var(--border-strong)', borderRadius: 9, padding: '8px 10px', color: 'var(--text)', fontSize: 13 }} value={testerId} onChange={e => setTesterId(e.target.value)}>
              {agents.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="set-body scroll">
          ...
          {!['llm', 'agents'].includes(sec) && (
            <div className="empty" style={{ height: '100%' }}>
              <Icon name={cur.ic} />
              <div>{cur.name}<br /><span style={{ fontSize: 12 }}>此面板为占位，可按需扩展</span></div>
            </div>
          )}
        </div>
      </div>
