...
import { AUDIT_PROJECTS, REQUIREMENTS, REPORT, DIFF_HUNKS, type AuditProject } from '../data/mock';

const SEV_COLOR: Record<string, string> = { critical: 'red', high: 'amber', medium: 'blue', low: 'green' };
...
function AuditList({ project, setProject, active, onSelect }: {
  ...
  setProject: (p: AuditProject) => void;
  active: string;
        ...
        {REQUIREMENTS.map(r => (
          <div key={r.id} className={'req-item' + (active === r.id ? ' active' : '')} onClick={() => onSelect(r.id)}>
            <div className="req-item-top">
              ...
              <span className={'chip ' + CAT_COLOR[r.cat]} style={{ padding: '1px 7px', fontSize: 10 }}>{r.cat}</span>
              <div style={{ marginLeft: 'auto', width: 8, height: 8, borderRadius: '50%', background: `var(--${r.sev === 'critical' ? 'red' : r.sev === 'high' ? 'amber' : 'blue'})` }} />
            </div>
...
function PreviewScreen({ variant }: { variant: 'prod' | 'cr' }) {
  const tags = variant === 'cr' ? ['全部', '待办', '已完成'] : null;
  return (
            ...
            <div className="scroll" style={{ flex: 1, overflowY: 'auto' }}>
              ...
              <div className="prev-frame" style={{ borderColor: 'var(--ember-tint-strong)' }}>
                <div className="prev-bar">
                  ...
                  <button className="icon-btn" style={{ width: 24, height: 24, marginLeft: 'auto' }} title="重启容器全量重载">
