/* ============================================================
   AutoForge — Conversations (IM: 1:1 + group + @mention)
   ============================================================ */

function ConvList({ active, onSelect, onNew }) {
  const [q, setQ] = useState('');
  const convs = AF.CONVERSATIONS;
  const directs = convs.filter(c => c.type === 'direct');
  const groups = convs.filter(c => c.type === 'group');
  const title = (c) => c.type === 'direct' ? AF.A[c.agent].name : c.name;
  const match = (c) => !q || title(c).toLowerCase().includes(q.toLowerCase());

  const Item = (c) => {
    const isG = c.type === 'group';
    const a = isG ? null : AF.A[c.agent];
    return (
      <div key={c.id} className={'conv-item' + (active === c.id ? ' active' : '')} onClick={() => onSelect(c.id)}>
        ...
        <div className="conv-main">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span className="conv-preview">{isG && <Icon name="bot" size={11} style={{ verticalAlign: -1, marginRight: 3, color: 'var(--text-faint)' }} />}{c.preview}</span>
...
function ChatHeader({ conv }) {
  ...
  const members = isG ? conv.members.map(id => AF.A[id]) : null;
  return (
    <div className="chat-head">
      ...
      <div className="chat-head-info">
        <div className="chat-head-title">
          {isG ? conv.name : AF.A[conv.agent].name}
          {isG && <span className="chip" style={{ padding: '1px 7px', fontSize: 10 }}>{conv.members.length} 成员</span>}
        </div>
        <div className="chat-head-sub">
          {isG ? <span style={{ fontFamily: 'var(--font-mono)' }}>{members.map(m => m.name).join(' · ')}</span>
               : <>{AF.A[conv.agent].en} · {AF.A[conv.agent].llm} · <span style={{ color: 'var(--green-soft)' }}>在线</span></>}
        </div>
      </div>
      <div className="chat-head-actions">
        {isG && <div className="member-stack" style={{ marginRight: 8 }}>
...
function MessageRow({ m, isGroup }) {
  const me = m.from === 'me';
  const a = me ? null : AF.A[m.from];
  return (
...
function Composer({ conv, onSend }) {
  const [text, setText] = useState('');
  const [showMention, setShowMention] = useState(false);
  const [mentionSel, setMentionSel] = useState(0);
  const taRef = useRef(null);
  const isG = conv.type === 'group';
  const members = isG ? conv.members.map(id => AF.A[id]) : [];
  ...
  const onKey = (e) => {
    if (showMention && (e.key === 'Enter' || e.key === 'Tab')) { e.preventDefault(); pickMention(members[mentionSel]); return; }
  ...
  return (
    <div className="composer">
      <div className="composer-tools">
        ...
        {isG && <button className="icon-btn" title="@ 指定 Agent" onClick={() => { setText(t => t + '@'); setShowMention(true); taRef.current && taRef.current.focus(); }}><Icon name="at" size={18} /></button>}
      ...
      <div className="composer-box" style={{ position: 'relative' }}>
        ...
        <textarea ref={taRef} rows={1} value={text} onChange={onChange} onKeyDown={onKey}
          placeholder={isG ? '输入消息，@ 可指定 Agent 回答…' : `给 ${AF.A[conv.agent].name} 发消息…`} />
...
function ChatView({ conv, msgs, onSend }) {
  const scrodRef = useRef(null);
  const isG = conv.type === 'group';
  useEffect(() => { const el = scrodRef.current; if (el) el.scrollTop = el.scrollHeight; }, [msgs, conv.id]);
  return (
    <div className="content">
      <ChatHeader conv={conv} />
      <div className="msgs scroll" ref={scrodRef}>
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,.5)', backdropFilter: 'blur(3px)', display: 'grid', placeItems: 'center', zIndex: 60 }} onClick={onClose}>
      <div className="rise" style={{ width: 420, background: 'var(--bg-2)', border: '1px solid var(--border-strong)', borderRadius: 18, boxShadow: 'var(--shadow-lg)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 }}>
            {AF.AGENTS.map(a => (
              <div key={a.id} className="mention-row" style={{ border: '1px solid ' + (sel.includes(a.id) ? 'var(--ember)' : 'transparent'), background: sel.includes(a.id) ? 'var(--ember-tint)' : 'transparent' }} onClick={() => toggle(a.id)}>
                ...
                <div className={'dot ' + (sel.includes(a.id) ? '' : 'gray')} style={sel.includes(a.id) ? { background: 'var(--ember)', width: 18, height: 18, borderRadius: 6, display: 'grid', placeItems: 'center' } : {}}>
        ...
        <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <button className="btn btn-primary" disabled={sel.length < 2} style={sel.length < 2 ? { opacity: .5 } : {}} onClick={() => onCreate(name, sel)}><Icon name="plus" size={15} />创建群聊</button>
        </div>
      </div>
    </div>
  );
...
function ConversationsPage() {
  ...
  const [store, setStore] = useState(AF.MESSAGES);
  const [showNew, setShowNew] = useState(false);
  const conv = AF.CONVERSATIONS.find(c => c.id === active);
  ...
  const send = (text) => {
    ...
    const isG = conv.type === 'group';
    let responderId = isG ? null : conv.agent;
    const mention = isG && text.match(/@([\u4e00-\u9fa5A-Za-z ]+)/);
    if (isG) {
      const found = mention && AF.AGENTS.find(a => mention[1].trim().startsWith(a.name));
      responderId = found ? found.id : conv.members[0];
    }
    ...
    setTimeout(() => {
      setStore(s => {
        ...
        const reply = isG
          ? `收到 @管理员 的消息。我是${AF.A[responderId].name}，已结合群聊上下文与附件分析，稍后给出结构化结论。`
          : '已收到，我正在处理你的请求并会同步进展。';
  ...
  return (
    <>
      ...
      {showNew && <NewGroupModal onClose={() => setShowNew(false)} onCreate={() => setShowNew(false)} />}
    </>
  );
}
