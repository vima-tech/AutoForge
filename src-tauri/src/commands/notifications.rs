...
pub async fn record_notification(db: &Db, ev: &AppEvent) -> Result<(), String> {
    let Some(draft) = NotificationDraft::from_event(ev) else {
        return Ok(());
    };
...
async fn insert_draft(db: &Db, draft: &NotificationDraft) -> Result<(), String> {
    let id = Uuid::new_v4().to_string();
    sqlx::query(
        "INSERT INTO notifications (id, category, kind, title, body, link_page, link_ref, read)
         VALUES (?, ?, ?, ?, ?, ?, ?, 0)",
    )
    .bind(&id)
    .bind(draft.category)
